...
public:
                         ...
                         int &temp_count, int &label_count) const override
    {
        ...
        string lhs_c = lhs->generate_code(outcode, symbol_to_temp, temp_count, label_count);
        string rhs_c = rhs->generate_code(outcode, symbol_to_temp, temp_count, label_count);
        if (symbol_to_temp[rhs_c] != "")
            outcode << lhs_c << " = " << symbol_to_temp[rhs_c] << endl;
        else
            outcode << lhs_c << " = " << rhs_c << endl;
...
public:
                         ...
                         int &temp_count, int &label_count) const override
    {
        ...
        for (auto argument : arguments)
        {
            ...
            if (symbol_to_temp[arg_t] != "")
            {
                string temp = "t" + to_string(temp_count);
                temp_count++;
                outcode << temp << " = " << symbol_to_temp[arg_t] << endl;
                outcode << "param " << temp << endl;
                temps.push_back(arg_t);
            }
            else
            {
                outcode << "param " << arg_t << endl;
                temps.push_back(arg_t);
            }
        }
